...
class Student extends Database
{
    ...
    public function fetchLoans($max_date=0, $min_date=0) {
        try {            
            $sql = "SELECT * FROM loans WHERE client_id != 0 AND loan_Id IS NULL  ";
    ...
    public function fetchStudent($id = 0) {
        try {            
            $sql = "SELECT *,CAST(REPLACE(loan_amount, ',', '') AS FLOAT) as amount,SUBSTRING(acct_no,1,CHAR_LENGTH(acct_no) - 1) as sub,STR_TO_DATE(loan_date, '%d-%b-%y') as day,REPLACE(loan_date,'-', ' ') as date_loan, STR_TO_DATE(loan_date, '%d-%b-%y') as day,REPLACE(exp_date,'-', ' ') as loan_expiry FROM loans ";
            ...
            foreach($rows as $s){
                // print_r($s);
                try{
                    $st = str_replace(' - ','',$s['tel']);
                    $sql2 = "SELECT * FROM m_client WHERE mobile_no ='$st'";
                    $stmt2 = $this->con_2->prepare($sql2);
                    // $stmt2 = $this->connect->prepare($sql2);            
                    $stmt2->execute();
                    $row_clients = $stmt2->fetchAll();
                    // var_dump($s['date_loan']);
                    ...
                    $loan_data = array(
                            ...
                            "locale"=> "en",
                            "clientId"=> $row_clients[0]['account_no'],
                            "productId"=> 5,
                            "principal"=> $s['amount'],
    ...
    //                 // var_dump($s['date_loan']);
    //                 $dateObject = DateTime::createFromFormat('d M y', $s['loan_date']);
    ...
    public function fetchApprove(){
        $sql = "SELECT *,CAST(REPLACE(loan_amount, ',', '') AS FLOAT) as amount,SUBSTRING(acct_no,1,CHAR_LENGTH(acct_no) - 1) as sub,STR_TO_DATE(loan_date, '%d-%b-%y') as day,REPLACE(loan_date,'-', ' ') as date_loan, STR_TO_DATE(loan_date, '%d-%b-%y') as day,REPLACE(exp_date,'-', ' ') as loan_expiry FROM loans_1_19jan WHERE client_id != 0 AND loan_id !=0 ";
